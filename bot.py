...
if pattern.search(text):
    ...
    # Ogohlantirish xabarini yuborish va 10 daqiqadan keyin o'chirish
    try:
        warning = await update.message.chat.send_message(
            f"⚠️ {name}, iltimos so'kinmang! Guruh qoidalariga rioya qiling. 🙏"
        )
        # ✅ TUZATISH: asyncio.create_task() o'rniga application.create_task() ishlatildi
        context.application.create_task(delete_after(warning, 10 * 60))
    except Exception:
        pass
```
...
if **name** == “**main**”:
# Keep-alive serverni alohida threadda ishga tushirish
